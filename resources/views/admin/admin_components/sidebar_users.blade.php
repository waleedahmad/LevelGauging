<div class="sidebar-user">
    ...
    <ul class="users-list">
    <?php
        $users      =   App\Models\User::where('type','=','user')->paginate(17);;
    ...
    @if($users->count())
        @foreach($users as $user)
            @if(isset($user_email) && $user_email === $user->email)
                ...
                <a href='/users/{{$user_email}}/{{$active}}?page={{$request->input('page')}}' class='single-user' data-status='{{$user->approved}}'>
                @else
            ...
            @else
                @if($request->input('page'))
                <a href='/users/{{$user->email}}/{{$active}}?page={{$request->input('page')}}' class='single-user' data-status='{{$user->approved}}'>
                @else
