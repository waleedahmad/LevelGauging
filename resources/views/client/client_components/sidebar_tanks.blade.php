<div class="sidebar-tanks">
    <ul class="tanks-list">
    <?php
        $email  =   App\Models\User::where('id','=',$user_id)->get()->first()->email;
        $tanks  =   App\Models\Tank::where('owner','=',$email)
                        ->orderBy('id','DESC')
                        ->lists('id');
    ?>
    @if($tank->count())
        @foreach($tanks as $id)
            <?php
                $tank_specs     =   App\Models\TankSpecs::where('tank_id','=',$id)->get()->first();
            ?>
            @if($id == $tank->id)
